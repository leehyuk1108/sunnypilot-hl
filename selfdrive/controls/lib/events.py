...
def startup_master_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int) -> Alert:
  ...
  return StartupAlert("개발중인 버전, 사용에 유의하세요", branch, alert_status=AlertStatus.userPrompt)
...
def modeld_lagging_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int) -> Alert:
  return NormalPermanentAlert("오픈파일럿이 불안정합니다", f"{sm['modelV2'].frameDropPerc:.1f}% 프레임 떨어짐")
...
EVENTS: Dict[int, Dict[str, Union[Alert, AlertCallbackType]]] = {
  ...
  EventName.steerTempUnavailable: {
    ET.SOFT_DISABLE: soft_disable_alert("조향이 일시적으로 비활성화됨"),
    ET.NO_ENTRY: NoEntryAlert("조향이 일시적으로 비활성화됨"),
  },
